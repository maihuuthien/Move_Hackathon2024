...
class EgoVehicle(Vehicle):
    ...
    def __init__(self, uid, name, parent, node, carla_actor, vehicle_control_applied_callback):
        ...
        self.add_force_subcriber = node.new_subscription(
            Vector3,
            self.get_topic_prefix() + "/vehicle_add_force",
            lambda data: self.control_add_force(data),
            qos_profile=10)
    ...
    def send_vehicle_msgs(self, frame, timestamp):
        ...
        vehicle_status.location.x = self.carla_actor.get_transform().location.x
        vehicle_status.location.y = self.carla_actor.get_transform().location.y
        vehicle_status.location.z = self.carla_actor.get_transform().location.z
        vehicle_status.rotation.roll = self.carla_actor.get_transform().rotation.roll
        vehicle_status.rotation.pitch = self.carla_actor.get_transform().rotation.pitch
        vehicle_status.rotation.yaw = self.carla_actor.get_transform().rotation.yaw
    ...
    def control_add_force(self, ros_force_add):
        force= carla.Vector3D(x=-1000, y = 0, z=0)
        self.carla_actor.add_force(force)
        # force= carla.Vector3D(x=ros_force_add.x+2, y = ros_force_add.y+2, z=ros_force_add.z+2)
