...
class CarlaRosBridge(CompatibleNode):
    ...
    def initialize_bridge(self, carla_world, params):
        ...
        ego_vehicles_in_world = list(carla_world.get_actors().filter(DReyeVR_vehicles))
        if len(ego_vehicles_in_world) >= 1:
            print(
                f"Found a DReyeVR EgoVehicle in the world ({ego_vehicles_in_world[0].id})"
            )
            self.actor_factory._create_object_from_actor(ego_vehicles_in_world[0])
            # self.player = 
            # print(self.player)
            # if 'vehicle' in self.player.type_id:
            #     # Get the 'role_name' attribute.
            #     role_name = self.player.attributes.get('role_name', 'No role_name attribute found')
            #     print(role_name)
            # else:
