...
class ActorFactory(object):
    ...
    def __init__(self, node, world, sync_mode=False):
        ...
        self.is_have_traffic_light_sensor = False
    ...
    def _create_object(self, uid, type_id, name, attach_to, spawn_pose, carla_actor=None):
        ...
        if type_id == TFSensor.get_blueprint_name():
            ...
            if carla_actor.type_id == "traffic.traffic_light":
                ...
                if not self.is_have_traffic_light_sensor:
                    actor_1 = TrafficLightsSensor(
                            uid=uid+200,
                            name=name,
                            parent=actor,
                            node=self.node,
                    ...
                    self.actors[actor_1.uid] = actor_1
                    self.is_have_traffic_light_sensor = True
            else:
                actor = Traffic(uid, name, parent, self.node, carla_actor)
        elif carla_actor.type_id.startswith("vehicle") or carla_actor.type_id.startswith("harplab.dreyevr_vehicle"):
            if carla_actor.attributes.get('role_name')\
                ...
                print(uid, " ", type_id, " ", name, " ", parent, " ", attach_to, " ", carla_actor, " ", spawn_pose)
            else:
        ...
        if carla_actor is not None:
            if not carla_actor.type_id.startswith("sensor.other.obstacle"): 
                # print(self.actors)
